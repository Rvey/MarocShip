...
import { useGetDriverQuery, useGetDriversQuery, useUpdateDriverMutation } from '../../../Redux/services/driver';
...
const UpdateDriverFrom: React.FC<UpdateDriverFormProps> = ({ setIsOpen, driverId }) => {

    const { refetch } = useGetDriversQuery();
    const { data, error, isLoading, refetch: re } = useGetDriverQuery(driverId);
    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                updateDriver({ id: driverId, ...values })
...
export default UpdateDriverFrom;
